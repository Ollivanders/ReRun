$(document).ready(function () { // displays all current events when page fully loaded due to .read()
    render($('#numberNodes').val(), $('#size').val(), $('#latitude').val(), $('#longitude').val());
});
...
function render(numberNodes, distanceParameter,latitudeParam, longitudeParam){
...
// Instantiate the map:
    ...
    var latitude = Number(latitudeParam);
    var longitude = Number(longitudeParam);
    var map = new H.Map(
    ...
    for (var i = 0; i < (n_waypoints + 1); i++) {
        ...
        routingParameters['waypoint' + i] = 'geo!' + lat + ',' + long;
    }
